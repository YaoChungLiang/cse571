...
class Field:
    ...
    def G(self, x, u):
        ...
        Gt=np.array([[1,0,-trans*np.sin(rot1+prev_theta)],
                     [0,1,trans*np.cos(rot1+prev_theta)],
                     [0,0,1]])
        return Gt
    ...
    def V(self, x, u):
        ...
        rot1, trans, rot2 = u.ravel()
        # YOUR IMPLEMENTATION HERE
        Vt=np.array([[-trans*np.sin(rot1+prev_theta), np.cos(rot1+prev_theta), 0],
                     [ trans*np.cos(rot1+prev_theta), np.sin(rot1+prev_theta), 0],
                     [                             1,                       0, 1]])
        return Vt
    ...
    def H(self, x, marker_id):
        ...
        prev_x, prev_y, prev_theta = x.ravel()
        # YOUR IMPLEMENTATION HERE
        #Ht=np.array([[x[0]/np.sqrt(x[0]^2+x[1]^2),x[1]/np.sqrt(x[0]^2+x[1]^2),0]])
        #Ht=np.array([[prev_x/np.sqrt(prev_x**2+prev_y**2),
        #              prev_y/np.sqrt(prev_x**2+prev_y**2),
        #              0]])
        
        #Ht=np.array([[(self.MARKER_X_POS[marker_id]-prev_x)/np.sqrt(prev_x**2+prev_y**2),
        #              (self.MARKER_Y_POS[marker_id]-prev_y)/np.sqrt(prev_x**2+prev_y**2),
        #              0]])
        dx=self.MARKER_X_POS[marker_id]-prev_x
        dy=self.MARKER_Y_POS[marker_id]-prev_y
        #Ht=np.array([[(dx)/np.sqrt(dx**2+dy**2),
        #              (-dy)/np.sqrt(dx**2+dy**2),
        #              -1]])
        #Ht=np.array([[(dx)/(dx**2+dy**2),
        #              (-dy)/(dx**2+dy**2),
        #              -1]])
        Ht=np.array([[(dy)/(dx**2+dy**2),
                      (-dx)/(dx**2+dy**2),
                      -1]])
        return Ht
    ...
    def sample_noisy_action(self, u, alphas=None):
        ...
        #print(np.random.multivariate_normal(u.ravel(), cov).reshape((-1, 1)))
