...
def localize(env, policy, filt, x0, num_steps, plot=False):
    ...
    for i in range(10):
        ...
        for i in range(num_steps):
            ...
            marker_id = env.get_marker_id(i) # 1 2 3 4 5 6 
            ...
            states_filter[i+1, :] = mean.ravel() # mean : 1 by 3
            # states_filter 201 by 3
...
def setup_parser():
    ...
    parser.add_argument(
        '--num-times', type=int, default=1,
        help='run n times to simulate')
...
if __name__ == '__main__':
    args = setup_parser().parse_args()       # read command
    ...
    alphas = np.array([0.05**2, 0.005**2, 0.1**2, 0.01**2]) # 1 by 4 array float
    beta = np.diag([np.deg2rad(5)**2]) # 1 float

    #times = args.num_times
    env = Field(args.data_factor * alphas, args.data_factor * beta) # carry the alphas and beta
    policy = policies.OpenLoopRectanglePolicy()

    initial_mean = np.array([180, 50, 0]).reshape((-1, 1)) # 3 by 1
    initial_cov = np.diag([10, 10, 1]) # 3 by 3
