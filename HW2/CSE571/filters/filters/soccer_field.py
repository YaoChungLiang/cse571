...
class Field:
    ...
    def G(self, x, u):
        ...
        Gt=np.array([[1,0,-trans*np.cos((rot1+prev_theta))],\
                     [0,1, trans*np.sin((rot1+prev_theta))],\
                     [0,0,                              1]])
        return Gt
    ...
    def H(self, x, marker_id):
        ...
        y=prev_y
        t=prev_theta
        #Ht=np.array([[(-x)/((marker_id[0]-x)^2+(marker_id[1]-y)^2),(y)/((marker_id[0]-x)^2+(marker_id[1]-y)^2),0]])
        Ht=np.array([[(-x)/((observer(x,marker_id)[0]-x)^2+(observer(x,marker_id)[1]-y)^2),(y)/((observer(x,marker_id)[0]-x)^2+(observer(x,marker_id)[1]-y)^2),0]])
        return Ht
