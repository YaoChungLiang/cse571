...
#
#                if self.planning_env.RRTstart_edge_validity_checker(plan[min_idx],next_point):
#                    
...
###################### ver 5 add color ####################################
        ...
        while last_point != goal_config:
            ...
            if numpy.random.rand()< self.bias:
                ...
                x_vec=candidate_next_point[0]-plan[min_idx][0]
                y_vec=candidate_next_point[1]-plan[min_idx][1]
                x_unit_vec=x_vec/len_candidate_next_point*4

                y_unit_vec=y_vec/len_candidate_next_point*4
        ...
        for i in range(((numpy.shape(original_tree)[0])//2)-1):
            ...
            y = [original_tree[j][1], original_tree[j+1][ 1]]
            plt.plot(y, x, 'g')
        plt.title('p=0.2, time = %.2f (s)' % total_time)
#        print(original_tree)
#        print(len(original_tree))
        return numpy.array(plan)
####################  ver 4  add bias  and   x y limit ######################################
#        plan=[]
...
################### ver 3  add  continuous limitation ##########################################
#        plan=[]
#        plan.append(start_config)
#        edge=dict()
#        dist=[]
